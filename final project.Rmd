...
data<-
  data %>%
  mutate(new_mental_health1 =case_when(
    ...
    TRUE ~ 0))
data1 <- data%>%mutate(new_mental_health = ifelse(self_rated_mental_health%in% states, "good", "not so good"))%>%dplyr::select(age, feelings_life, new_mental_health, income_family, self_rated_health,living_arrangement,worked_last_week,new_mental_health1)
...
# Abstract
  ...
  The purpose of this report is to evaluate whether people think they have a good mental health based on a collection of factors: age, feelings of life, if they worked last week and physical health. The data session summarizes the dataset used in this analysis, and key features are included. In the modeling session, the logistic regression modeling is used in analysing the probability of having a good mental health, and there are compelling findings coming out of the result using the method difference-in-difference and the matching propensity score. Interestingly, as for all four factors, they are all negatively correlated with evaluation of mental health, which helps the society to better predict one’s mental health.

**Key words**: logistic modeling, logistic regression estimation, difference-in-difference, matching propensity score, negative correlation
...
Here, the variable `worked_last_week` is binary which means we can use the `difference-in-difference` and `matching propensity score` methods to explore more about the relationship between the `worked_last_week` and `new_mental_health`. The casual inference between the self rated mental health and the workload should also be analyzed since as the above result shown, if people worked last week would decrease the rate of mental health, however, there were many factors may affect the trend, for example, between young people and old people, if they all worked last week adn the rate of the mental health increases, this could not tell any story, because old people tend to love their jobs more than younger people, they may be more resilient at work, and therefore have higher levels of mental health. Therefore, I chose to use `Difference-in-Difference`here to discover the casual inference between the self rated mental health and the workload when the age is fixed, and then I got the graph below.

```{r echo=FALSE, message=FALSE, warning=FALSE}
diff_in_diff_example_data=data1
diff_in_diff_example_data$treatment_group <- 
  data1$worked_last_week

diff_in_diff_example_data$year <- 
  ifelse(data1$age >= 30, 1, 0)
```
...
diff_in_diff_example_data %>% 
  ggplot(aes(x = year,
             y = data1$new_mental_health1,
             color = as.factor(treatment_group))) +
  geom_point() +
  geom_line(aes(group = nrow(data1)), alpha=10) +
  theme_minimal() +
  labs(x = "Age",
       y = "Mental health",
       color = "Person worked last week") +
  scale_color_brewer(palette = "Set1")
```
**Figure 4:** This figure demonstrate the difference between how `worked_last_week` effects the `mental_health` when the `age` was controlled. The x-axis is the `age` variable and if the age is higher than 30, it turns out 1, and others will turns out to be 0. The y-axis is the `new_mental_health1` variable. Here, I group them into five groups from "Excellent" to "Poor" and the rates are 0 to 5. The blue lines were people who worked last week and the red one is not. From the figure, we could find out that, when people in the same age group, people who worked last week would have slightly higher rate of mental health than others who do not.

We also found that the average difference is 0.56 which means on average, when the age is fixed, the people who worked last week would have 0.56 higher self rated mental health than others who do not. Then, we could find out what is the exact number which discribe the relationship between the `new_mental_health` and the `worked_last_week`.
```{r echo=FALSE, message=FALSE, warning=FALSE}

diff_in_diff_example_regression <- 
  lm(data1$new_mental_health1 ~ treatment_group*year, 
     data = diff_in_diff_example_data)

kable(tidy(diff_in_diff_example_regression), caption= "Difference-in-Difference Result")

```

This table shows that when people in same age group, if they worked last week, the self rated mental health will be 0.0084 higher than they do not.

Explore more about how the `worked_last_week` effect the `new_mental_health`, and this time we control all other variables fixed and check what will the `worked_last_week` changes the `new_mental_health`, which is the method of `Matching-Propensity-Score`.

```{r echo=FALSE, message=FALSE, warning=FALSE}
propensity_score <- glm(new_mental_health ~ age + feelings_life +self_rated_health+worked_last_week, 
                        family = binomial,
                        data = data1)
...
prop_data <- 
  prop_data %>% 
  arrange(.fitted, new_mental_health)
prop_data$treated <- 
  data1$worked_last_week

prop_data$treated <- 
  as.integer(prop_data$treated)

matches <- arm::matching(z = prop_data$treated, 
                         score = prop_data$.fitted)

prop_data <- cbind(prop_data, matches)
...
propensity_score_regression <- 
  lm(new_mental_health1 ~ age + feelings_life + self_rated_health + worked_last_week, 
                data = prop_data_matched)

kable(huxtable::huxreg(propensity_score_regression), caption="Mathching Propensity Score Result")

```

Here is what we got if we control the `age`, `feelings_life`, and `self_rated_health` fixed, the `worked_last_week` would increase 0.079 in the `new_mental_health`, and we could also found that the `p-value` is (0.013) which is smaller than the 0.5, so this is a significant predictor to the `new_mental_health`.
...
Any missing values in the data were imputed as “Not Stated” or “Don’t Know”. While the assumption under which these imputations were conducted was probably reasonable, it may have been useful to perform a sensitivity analysis to determine whether such an imputation had a drastic effect on the results of the analyses. Moreover, here are some variables that have an impact on mental health in real life, such as individual's marital status, living environment and educational background, etc. However, because the values were too small in the p-test process, they were considered as insignificant variables, so they were deleted when the model was built. However, perhaps these variables have some influence on the model, and if deleted, some information may be missed. Here, the predictor `self_rated_mental_health` was rated by the responsers themselves, and the outcomes were just the scores from 1 to 10 which were subjectivity in this measurement, and this may effect the result of our final model since the numbers may not accurate to their exact mental health level.

Here, I only choose to have a more in-depth discussion on the impact of workload on mental health when other factors remain the same. I do not have a deeper discussion on the other three factors, and many outcomes may be more different if we understand them. At the same time, there must be more than these three factors affecting the work, maybe there are more and more important factors that I did not include, which may also affect the final result. 
...
Gss31_user_Guide of 2017 General Social Survey (GSS): Families Cycle 31 https://sda-artsci-utoronto-ca.myaccess.library.utoronto.ca/sdaweb/dli2/gss/gss31/gss31/more_doc/GSS31_User_Guide.pdf
...
(2018, March 11). Logistic Regression Assumptions and Diagnostics in R. Articles - STHDA. http://www.sthda.com/english/articles/36-classification-methods-essentials/148-logistic-regression-assumptions-and-diagnostics-in-r/

Population Data Source: Statistics Canada. Table 17-10-0005-01 Population estimates on July 1st, by age and sex  DOI: https://doi.org/10.25318/1710000501-eng.

Neverknowhen. (n.d.). Neverknowhen/Towards-a-logistic-analysis-of-satisfication-of-lifes. Retrieved December 21, 2020, from https://github.com/neverknowhen/Towards-a-logistic-analysis-of-satisfication-of-lifes
...
plot(mysvyglm1, which = 4, id.n = 10, main = "Observations with their Cook's Distance", sub.caption = "Figure 7")
